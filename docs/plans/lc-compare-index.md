...
> **계획 형식 예외 (사유 명시)**: 실험/비교 목적 코드 — 외부 API(LangChain v1)는 계획 시점 스모크 테스트로 실측 확정했고(아래), 본 엔진과 완전 격리되며 단위 테스트를 면제(LangChain 객체 mocking 비용이 학습 가치 대비 명백히 큼 — 실행 AC로 갈음)하므로 Haiku 위임 대신 메인 세션이 직접 구현한다. 본 엔진 테스트 114개 무접촉이 회귀 게이트.
